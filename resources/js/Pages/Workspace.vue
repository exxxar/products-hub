        ...
        <div class="workspace-container container">
            ...
            <div class="workspace-layout">
                ...
                <WorkspaceSidebar
                    ...
                    @switch-to-collection="viewMode = 'collections'"
                    @switch-to-category="viewMode = 'categories'"
                ...
                <div class="workspace-content">
                    ...
                    <template v-else>
                        ...
                        <!-- Режим коллекций -->
                        <template v-else-if="viewMode === 'collections'">
                            <CollectionGrid
                                ...
                                @toggle-stop-list="handleToggleCollectionStopList"
                                @view-products="handleViewCollectionProducts"
                        ...
                        <template v-else-if="viewMode === 'categories'">
                            <div class="categories-view">
                                ...
                                <!-- ===== Детальный просмотр выбранной категории ===== -->
                                <template v-if="selectedCategory">
                                    <div class="category-header-with-back">
                                        <button type="button" class="btn-back" @click="onBackToCategories">
                                    ...
                                    <ProductGrid
                                        ...
                                        :selectedIds="store.selectedIds"
                                        @create-product="openCreateProduct(selectedCategory?.id)"
                                        @edit-product="openEditProduct"
                                        @toggle-select="toggleSelect"
                                        @toggle-stop-list="handleToggleStopList"
                                        @edit-images="openImagesModal"
                                ...
                                <!-- ===== Список категорий (только названия + счётчики с бэка) ===== -->
                                <template v-else>
                                    ...
                                    <div
                                        ...
                                        :key="cat.id"
                                        class="category-group category-group--clickable"
                                        @click="onSelectCategory(cat)"
                                    >
                                        <div class="category-header">
                                            <h6 class="category-name">{{ cat.name }}</h6>
                                            <span class="category-count">
                            {{ cat.products_count ?? 0 }} {{ pluralize(cat.products_count ?? 0, 'товар', 'товара', 'товаров') }}
                        </span>
                                        </div>

                                        <div class="category-preview">
                                            <i class="fa-solid fa-arrow-right"></i>
                                            <span>Нажмите, чтобы открыть товары категории</span>
                                        </div>
                                    </div>
                                </template>
                            </div>
        ...
        methods: {
                ...
                try {
                    ...
                    if (!result?.success) {
                        if (result?.reason === 'not_found') {
                            this.$notify?.warning('Товар не найден в текущем списке')
                        }
                        return
                    }
                    this.$notify?.success({
            ...
            async initWorkspace() {
              ...
              //  this.store.setDisplayMode( 'workspaces')
               // this.store.setProducts(this.item.products || [])
                ...
                await this.store.loadCategories() // ✅ ДОБАВИТЬ — чтобы получить products_count
            ...
            openCreateProduct(categoryId = null) {
                ...
                if (this.$refs.productModal) {
                    this.$refs.productModal.show(categoryId) // Передаем ID категории
                }
            },
            ...
            async onSelectCategory(category) {
                ...
                if (category) {
                    ...
                    try {
                        await this.loadProductsByCategory(category.id) // теперь async/await работает
                    } catch (error) {
            ...
            async loadProductsByCategory(categoryId) {
                // ✅ теперь async
                await this.store.loadProductsByCategoryId(categoryId)
            },
            ...
            getProductsByCategory(categoryId) {
                ...
                if (!this.store.products || this.store.products.length === 0) {
                    return [];
                }


                const filtered = this.store.products.filter(p => {
                    // Защита от товаров без категорий
                    if (!p.categories || !Array.isArray(p.categories)) {
                        return false;
                    }

                    const hasCategory = p.categories.some(sc => {
                        const scId = (typeof sc === 'object' && sc !== null) ? sc.id : sc;
                        const scIdNumber = Number(scId);
                        const matches = scIdNumber === targetId;
            ...
            async handleToggleCollectionStopList(collection) {
                try {
            ...
            handleViewCollectionProducts(collection) {
                this.$refs.collectionProductsModal.show(collection)
            },
